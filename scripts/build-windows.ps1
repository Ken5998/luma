...
try {
    ...
    if (!(Test-Path -LiteralPath $cargo)) { throw 'Rust/Cargo non trovato. Installa Rust tramite rustup.' }
    & $cargo build --locked --release -p luma-desktop
    if ($LASTEXITCODE -ne 0) { throw 'Compilazione Luma fallita.' }
    $metadata = & $cargo metadata --locked --no-deps --format-version 1 | ConvertFrom-Json
    if ($LASTEXITCODE -ne 0) { throw 'Lettura metadata Cargo fallita.' }
    ...
    Write-Host "Pronto: $(Join-Path $release 'Luma.scr')"
